...
def parse_fcd_header(filepath):
    ...
    for ci, name in enumerate(cols):
        ...
        # Voltage — stack voltage
            ...
            result['v_col'] = ci
        # HFR — non-area-normalized (mOhm), from EIS
        elif n == 'HFR (mOhm)':
            result['hfr_col'] = ci
        # Current-interrupt resistance (mOhm) — carried through as a
        # pseudo-condition column so it flows through dwell extraction,
        # dedup, and cycle selection alongside the data, then pulled back
        # out for a second iR-correction / HFR overlay.
        elif n == 'E_iR_Stack (mOhm)':
            cond_cols['CI_HFR (mOhm)'] = ci
        # EIS columns
...
def attach_ci_hfr(results, geo_area=5.0):
    """
    Pull the current-interrupt resistance series out of results['conditions']
    (carried under key 'CI_HFR (mOhm)'), convert mΩ → Ω, and compute a second
    iR-free voltage curve for direct comparison against the EIS HFR.

    Adds to results:
      'CI_HFR'        — per-cell resistance in Ω (current interrupt)
      'CI_HFR_ASR'    — area-specific resistance in Ω·cm²
    ...
    results.setdefault('CI_HFR_ASR', None)
    results.setdefault('CI_HFR_mean', None)
    results.setdefault('V_irfree_CI', None)
    results.setdefault('P_irfree_CI', None)

    conditions = results.get('conditions')
    if not conditions or 'CI_HFR (mOhm)' not in conditions:
        return results

    ci_mohm = np.asarray(conditions['CI_HFR (mOhm)'], dtype=float)
    if ci_mohm.size == 0:
        return results

    j = results['j']
    V = results['V']
    if len(ci_mohm) != len(j):
        # Length mismatch — can't align; bail out safely
        return results
    ...
    results['CI_HFR_ASR'] = ci_asr
    results['CI_HFR_mean'] = float(np.mean(ci_asr))

    V_irfree_ci = compute_ir_free_voltage(V, j, ci_asr)
...
def plot_polcurve(results, save_path=None):
    ...
    V = results['V']
    P = results['P']
    has_hfr = (results['HFR'] is not None) or (results.get('CI_HFR') is not None)
    ...
    if results['V_irfree'] is not None:
        ax.plot(j, results['V_irfree'], 's--', color='green', ms=3, lw=1.2,
                alpha=0.7, label='iR-free (EIS HFR)')
    ...
    # ── Panel: HFR (EIS + current-interrupt overlaid) ──
    if has_hfr:
        ax = axes[panel]
        # readout labels carry KEY = VALUE so they appear in the comparison
        # readout box and Excel metrics
        if results['HFR_ASR'] is not None:
            ax.plot(j, results['HFR_ASR'] * 1000, 'o-', color='darkorange',
                    ms=4, lw=1.5,
                    label=f'EIS HFR = {results["HFR_mean"]*1000:.1f} mΩ·cm²')
        if results.get('CI_HFR_ASR') is not None:
    ...
    # Hide the carried CI_HFR pseudo-condition from the condition label
    cond_for_label = results.get('conditions')
    if cond_for_label and 'CI_HFR (mOhm)' in cond_for_label:
        cond_for_label = {k: v for k, v in cond_for_label.items()
                          if k != 'CI_HFR (mOhm)'}
    cond_label = get_condition_label(
        label=results.get('label', ''),
        conditions=cond_for_label)
    title = 'Polarization Curve Analysis — HFR Comparison (EIS vs Current Interrupt)'
    if cond_label:
        title += f'\n{cond_label}'
    ...
    if save_path:
        save_with_sidecar(fig, save_path, plot_type='polcurve_hfrcompare', dpi=200, bbox_inches='tight')
...
def plot_polcurve_overlay(all_results, save_path=None):
    ...
    def _clean_cond(r):
        c = r.get('conditions')
        if c and 'CI_HFR (mOhm)' in c:
            return {k: v for k, v in c.items() if k != 'CI_HFR (mOhm)'}
        return c
    ...
    for i, r in enumerate(all_results):
        ...
        cond = get_condition_label(label=lbl, conditions=_clean_cond(r), compact=True)
    ...
    fig.suptitle('Polarization Curve Overlay — HFR Comparison', fontsize=13, fontweight='bold')
    fig.tight_layout()

    if save_path:
        save_with_sidecar(fig, save_path, plot_type='polcurve_hfrcompare_overlay', dpi=200, bbox_inches='tight')
        print(f'  Saved: {save_path}')
    return fig
...
def save_single_excel(results, filepath):
    ...
    if cond:
        for name, vals in cond.items():
            if name == 'CI_HFR (mOhm)':
                continue  # already surfaced as Mean CI HFR; skip raw series
...
def save_batch_excel(all_results, summary_rows, filepath):
    ...
    for r in all_results:
        ...
        def _cyc_ncols(cyc):
            n = 3  # j_A, j_mA, V
            if cyc.get('HFR') is not None:
                n += 1
            if cyc.get('conditions') and 'CI_HFR (mOhm)' in cyc['conditions']:
                n += 1
            return n
        total_cycle_cols = sum(_cyc_ncols(cyc) for cyc in cycles)
        ...
        for ci, cyc in enumerate(cycles):
            ...
            cyc_ci = None
            if cyc.get('conditions') and 'CI_HFR (mOhm)' in cyc['conditions']:
                cyc_ci = cyc['conditions']['CI_HFR (mOhm)']
            n_cols = _cyc_ncols(cyc)
            ...
            hdrs = ['j (A/cm²)', 'j (mA/cm²)', 'V (V)']
            if cyc_hfr is not None:
                hdrs.append('EIS HFR (mΩ·cm²)')
            if cyc_ci is not None:
                hdrs.append('CI HFR (mΩ·cm²)')
            for hi, h in enumerate(hdrs):
            ...
            for ri in range(len(cyc_j)):
                ...
                if cyc_hfr is not None and ri < len(cyc_hfr):
                    ws2.cell(row=ri + 4, column=cc,
                             value=round(float(cyc_hfr[ri]) * geo * 1000, 2)); cc += 1
                if cyc_ci is not None and ri < len(cyc_ci):
                    # CI HFR carried in mΩ (per-cell) → ASR mΩ·cm²
                    ws2.cell(row=ri + 4, column=cc,
                             value=round(float(cyc_ci[ri]) * geo, 2))
...
def run_batch(filepaths, labels, geo_area,
    ...
    for i, (fp, lbl) in enumerate(zip(filepaths, labels)):
        ...
        try:
            ...
            # Pull current-interrupt resistance out of conditions and compute
            # its iR-free voltage for direct comparison with EIS HFR.
            attach_ci_hfr(r, geo_area=geo_area)
            ...
            if save_dir and image_ext:
                ...
                plot_polcurve(r, save_path=os.path.join(save_dir, f'polcurve_hfrcompare_{safe_name}.{image_ext}'))
                plt.close()
            ...
            # Add conditions to summary (skip the carried CI_HFR pseudo-column)
            if cond:
    ...
    if save_dir and summary_rows:
        csv_path = os.path.join(save_dir, 'polcurve_hfrcompare_batch_summary.csv')
        all_cols = list(summary_rows[0].keys())
        for row in summary_rows[1:]:
            for k in row:
                if k not in all_cols:
    ...
    if save_dir and all_results:
        xlsx_path = os.path.join(save_dir, 'polcurve_hfrcompare_batch_data.xlsx')
        save_batch_excel(all_results, summary_rows, xlsx_path)

    # ── Combined overlay ──
    if save_dir and image_ext:
        overlay_path = os.path.join(save_dir, f'polcurve_hfrcompare_batch_overlay.{image_ext}')
